  ...
  private void Update()
  {
    if (Input.GetMouseButtonDown(0))
    {
      ...
      if (path != null)
      {
        ...
        {
          Debug.DrawLine(new Vector3(path[i].GridPosition.x, path[i].GridPosition.y) + Vector3.one * .5f,
          new Vector3(path[i + 1].GridPosition.x, path[i + 1].GridPosition.y) + Vector3.one * .5f, Color.green, 3f, false);
        }
      }
    }
